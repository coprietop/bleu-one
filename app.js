...
// ===== Bleu One v1.8 · Gimnasio 4 en 14 =====
(()=>{
  const screen=document.getElementById('experto414');
  ...
  function renderObjection(){optionGame('Laboratorio de objeciones','Objeción · '+pick(objections).cat,pick(objections),'objections',renderObjection)}
  function renderObjectionFixed(){const item=pick(objections);optionGame('Laboratorio de objeciones','Objeción · '+item.cat,item,'objections',renderObjectionFixed)}
  function renderMarket(){const item=pick(markets);stage.innerHTML=`<div class="game-head"><div><span class="game-kicker">Juego de mercados</span><h3>${item.name}</h3></div><span class="badge">Califica antes de agendar</span></div><div class="game-card"><div class="profile-chips">${item.chips.map(x=>`<span>${x}</span>`).join('')}</div><div class="game-question">${item.q}</div><div class="game-options"></div><div class="game-feedback" hidden></div><div class="bleu-advice" hidden><strong>💎 Bleu te aconseja</strong><span>Una visita aceptada no siempre es una visita productiva. Protege tu tiempo.</span></div><button class="primary-action next-game" hidden>Otro perfil <span>→</span></button></div>`;const box=stage.querySelector('.game-options'),fb=stage.querySelector('.game-feedback'),next=stage.querySelector('.next-game'),ad=stage.querySelector('.bleu-advice');item.opts.forEach((o,i)=>{const b=document.createElement('button');b.textContent=o;b.onclick=()=>{[...box.children].forEach(x=>x.disabled=true);const good=i===item.a;b.classList.add(good?'correct':'wrong');if(!good)box.children[item.a].classList.add('correct');fb.hidden=false;fb.className='game-feedback '+(good?'good':'bad');fb.textContent=(good?'✓ ':'⚠ ')+item.why;ad.hidden=false;next.hidden=false;updateMastery('markets',good)};box.appendChild(b)});next.onclick=renderMarket}
  function renderError(){const item=pick(errors);optionGame('Detecta el error','Conversación real',item,'instant',renderError)}
  function renderSpeech(){const item=pick(speeches);optionGame('Completa el speech','Speech de Christian Prieto',item,'instant',renderSpeech)}
  ...
  function renderTimer(){stage.innerHTML=`<div class="game-head"><div><span class="game-kicker">Entrena tu instantánea</span><h3>Speech guiado contra el tiempo</h3></div><span class="badge">45 segundos</span></div><div class="game-card"><p>Habla en voz alta. Bleu irá mostrando la parte del speech que corresponde según el tiempo.</p><div class="timer-display" id="speechTimer">00:45</div><div class="timer-cue" id="timerCue">Pulsa comenzar y habla con naturalidad.</div><div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap"><button class="primary-action" id="timerStart">Comenzar <span>→</span></button><button class="small-action" id="timerReset">Reiniciar</button></div><div class="bleu-advice"><strong>💎 Bleu te aconseja</strong><span>No busques perfección. Busca seguridad, claridad y acción.</span></div></div>`;let left=45,t=null;const cues=[[42,'Pregunta: “¿A usted le gustaría que los visitara yo que ya me conoce o un desconocido?”'],[35,'Haz una pausa y pregunta: “¿Por qué?”'],[29,'Explica que, si el programa va a telemercadeo, las visitas pueden asignarse a otro asesor.'],[20,'Da la instrucción: “Vamos a llamar de una vez.”'],[13,'Indica al cliente: salude, presente y entregue el teléfono.'],[7,'Agenda. No expliques de más. Hoy, mañana o máximo pasado mañana.']];const display=stage.querySelector('#speechTimer'),cue=stage.querySelector('#timerCue'),start=stage.querySelector('#timerStart');function tick(){left--;display.textContent=`00:${String(left).padStart(2,'0')}`;const found=cues.find(x=>x[0]===left);if(found)cue.textContent=found[1];if(left<=0){clearInterval(t);cue.textContent='✓ Entrenamiento terminado. ¿Lograste hablar con seguridad y sin explicar demasiado?';start.disabled=false;start.textContent='Repetir';updateMastery('instant',true)}}start.onclick=()=>{clearInterval(t);left=45;display.textContent='00:45';cue.textContent='Comienza con la pregunta de confianza.';start.disabled=true;t=setInterval(tick,1000)};stage.querySelector('#timerReset').onclick=renderTimer}
  function renderMentor(){stage.innerHTML=`<div class="game-head"><div><span class="game-kicker">Guía práctica para formar</span><h3>💎 Modo Mentor</h3></div><span class="badge">Para asesores nuevos</span></div><div class="game-card"><p>Usa esta ruta durante una práctica presencial. El nuevo asesor explica y ejecuta cada paso antes de continuar.</p><div class="mentor-roadmap">${steps.map((s,i)=>`<div class="mentor-step"><b>${i+1}</b><div><h4>${s}</h4><p>${['Haz preguntas para aliviar el estrés post negociación.','Genera deseo y confirma qué premio elegiría.','Diferencia pedir nombres de simplemente anotarlos. Prioriza el primer círculo.','Valida ocupación, núcleo familiar, hábitos y potencial de mercado.','Comprueba que entiende: 4 visitas, mínimo 1 venta y 14 días.','Practica llamada inmediata, activación por voz y transferencia de confianza.','Define la próxima acción dentro de 48 horas.','Reemplaza, reactiva y recupera oportunidades antes de abandonar.'][i]}</p></div></div>`).join('')}</div><div class="bleu-advice"><strong>💎 Bleu te aconseja</strong><span>El mentor no hace el programa por el asesor. Lo observa, corrige y lo lleva a repetir hasta que el orden salga de forma natural.</span></div></div>`}
  function renderRandom(){const modes=[renderObjectionFixed,renderMarket,renderError,renderOrder,renderSpeech];pick(modes)()}
  const renderers={random:renderRandom,objections:renderObjectionFixed,markets:renderMarket,errors:renderError,order:renderOrder,speech:renderSpeech,timer:renderTimer,mentor:renderMentor};
  document.getElementById('startGym')?.addEventListener('click',renderRandom);
  screen.querySelectorAll('.gym-mode').forEach(btn=>btn.addEventListener('click',()=>{screen.querySelectorAll('.gym-mode').forEach(x=>x.classList.toggle('active',x===btn));renderers[btn.dataset.gym]()}));
