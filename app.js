...
(()=>{
  ...
  const levelWeight={beginner:2,intermediate:5,expert:10};
  const levelCap={beginner:35,intermediate:70,expert:100};
  function makeChoices(best,others){return shuffle([best,...others]).map((text,i)=>({text,best:text===best}));}
  function difficultyIntro(level){return level==='beginner'?'Casos directos, señales visibles y retroalimentación amplia.':level==='intermediate'?'Opciones muy parecidas: deberás identificar la intención más sólida.':'Todas las opciones pueden funcionar; solo una protege mejor la relación, el proceso y el resultado.'}
  function buildQuestion(base,level,seed=0){
    const variants={
      beginner:[base.best,base.good,base.ok,base.risky],
      intermediate:[base.good,base.best,base.ok,base.risky],
      expert:[base.good,base.ok,base.best,base.risky]
    };
    return {q:base.q,context:base.context||'',choices:makeChoices(base.best,variants[level].filter(x=>x!==base.best)),why:base.why,psych:base.psych||'',tip:base.tip||'',level,seed};
  }
  class SessionMastery{
    constructor(prefix,areas){this.prefix=prefix;this.areas=areas;this.data={};areas.forEach(a=>this.data[a]={xp:0,correct:0,total:0});}
    add(area,good,level){const d=this.data[area];d.total++;if(good)d.correct++;d.xp+=good?levelWeight[level]:1;this.paint();}
    areaPct(a){const d=this.data[a];if(!d.total)return 0;const accuracy=d.correct/d.total*100;const xp=Math.min(100,d.xp/5);return Math.round(accuracy*.72+xp*.28)}
    totalPct(){const vals=this.areas.map(a=>this.areaPct(a));return Math.round(vals.reduce((a,b)=>a+b,0)/vals.length)}
    paint(){this.areas.forEach(a=>{const el=document.getElementById(this.prefix+'Bar'+a[0].toUpperCase()+a.slice(1));if(el)el.style.width=this.areaPct(a)+'%'});const s=document.getElementById(this.prefix+'MasteryScore');if(s)s.textContent=this.totalPct()+'%';}
  }
  function renderHow(title,desc,evals,level){return `<div class="how-game"><span>¿CÓMO FUNCIONA?</span><h4>${title}</h4><p>${desc}</p><div class="how-tags">${evals.map(x=>`<i>${x}</i>`).join('')}</div><small>${difficultyIntro(level)}</small></div>`}
  function renderMCQ(stage,item,onAnswer,next,label='Siguiente caso'){
    stage.querySelector('.question-zone').innerHTML=`${item.context?`<div class="case-context">${item.context}</div>`:''}<div class="game-question">${item.q}</div><div class="game-options"></div><div class="game-feedback" hidden></div><div class="expert-insight" hidden><article><span>⭐ Por qué esta es la mejor</span><p>${item.why}</p></article>${item.psych?`<article><span>🧠 Psicología del cliente</span><p>${item.psych}</p></article>`:''}${item.tip?`<article><span>💡 Tip para la práctica</span><p>${item.tip}</p></article>`:''}</div><button class="primary-action next-v3" hidden>${label} <span>→</span></button>`;
    const box=stage.querySelector('.game-options'),fb=stage.querySelector('.game-feedback'),ins=stage.querySelector('.expert-insight'),btn=stage.querySelector('.next-v3');
    item.choices.forEach((c,i)=>{const b=document.createElement('button');b.innerHTML=`<b>${String.fromCharCode(65+i)}</b><span>${c.text}</span>`;b.onclick=()=>{[...box.children].forEach(x=>x.disabled=true);b.classList.add(c.best?'correct':'wrong');const best=[...box.children].find((_,j)=>item.choices[j].best);best?.classList.add('correct');fb.hidden=false;fb.className='game-feedback '+(c.best?'good':'bad');fb.innerHTML=c.best?'✓ <strong>Decisión de mayor calidad.</strong> Las otras opciones podían ayudar, pero esta protege mejor todo el proceso.':'⚠ <strong>Era una opción defendible, pero no la más completa.</strong> Compara intención, momento y efecto.';ins.hidden=false;btn.hidden=false;onAnswer(c.best,item);};box.appendChild(b)});btn.onclick=next;
  }

  // ---- Gimnasio 4 en 14 reconstruido ----
  const gym=document.getElementById('gimnasio414');
  if(gym){
    gym.innerHTML=`<section class="panel gym-hero"><div><span class="eyebrow">Entrena, repite, mejora</span><h3>🏋️ Gimnasio 4 en 14</h3><p>Práctica deliberada para dominar el orden, el criterio, los speeches y la psicología detrás del sistema.</p></div><div class="mastery-ring"><strong id="g414MasteryScore">0%</strong><span>Maestría de esta sesión</span></div></section>
    <section class="panel difficulty-panel"><div><span class="eyebrow">Nivel de entrenamiento</span><h3>Elige tu dificultad</h3><p>El nivel experto no se obtiene con cinco aciertos: exige consistencia y muchas decisiones correctas.</p></div><div class="difficulty-switch" id="g414Difficulty"><button data-level="beginner" class="active">Principiante</button><button data-level="intermediate">Intermedio</button><button data-level="expert">Experto</button></div></section>
    <div class="mastery-bars panel"><div><span>Orden</span><i><b id="g414BarOrder"></b></i></div><div><span>Objeciones</span><i><b id="g414BarObjections"></b></i></div><div><span>Mercados</span><i><b id="g414BarMarkets"></b></i></div><div><span>Instantánea</span><i><b id="g414BarInstant"></b></i></div><small>Progreso temporal de esta sesión. Para acercarte al 100% necesitas precisión sostenida en niveles altos.</small></div>
    <div class="gym-menu"><button class="gym-mode active" data-g414="objections">🧪 Laboratorio de objeciones</button><button class="gym-mode" data-g414="markets">📊 Juego de mercados</button><button class="gym-mode" data-g414="errors">🚨 Detecta el error</button><button class="gym-mode" data-g414="order">🧩 Orden perfecto</button><button class="gym-mode" data-g414="speech">🎤 Completa el speech</button><button class="gym-mode" data-g414="timer">⏱️ Entrena tu instantánea</button><button class="gym-mode" data-g414="mind">🧠 Piensa como un experto</button><button class="gym-mode" data-g414="quick">⚡ Reto 10 segundos</button><button class="gym-mode" data-g414="mentor">💎 Modo Mentor</button></div>
    <section class="panel gym-stage" id="g414Stage"></section>`;
    let level='beginner', current='objections',counter=0;
    const mastery=new SessionMastery('g414',['order','objections','markets','instant']);
    const stage=document.getElementById('g414Stage');
    const bases=[
      {q:'El cliente dice: “No quiero molestar a mi familia”. ¿Cuál es la mejor intervención?',best:'Validar su preocupación, explicar que la confianza familiar protege la experiencia y preguntarle quién recibiría mejor un detalle de su parte.',good:'Aclarar que nadie está obligado a comprar y pedirle dos familiares cercanos.',ok:'Ofrecer comenzar con amigos y después regresar a la familia.',risky:'Aceptar solo compañeros de trabajo para no generar resistencia.',why:'Valida la emoción, explica el porqué y conduce hacia el primer círculo sin imponer.',psych:'Está protegiendo sus relaciones y teme quedar mal.',tip:'No discutas la objeción; convierte la confianza en el argumento principal.'},
      {q:'El cliente dice: “Yo los llamo mañana”. ¿Qué decisión protege mejor la instantánea?',best:'Reconocer su intención y liderar una llamada breve ahora para dejar fecha, hora y dirección confirmadas.',good:'Pedirle que envíe un audio ahora y acordar seguimiento mañana.',ok:'Dejar dos horarios tentativos para que el prospecto escoja después.',risky:'Aceptar y recordarle al día siguiente.',why:'La confianza y la emoción son más altas en el momento de la demostración.',psych:'Busca salir del compromiso sin decir “no”.',tip:'La instantánea termina con una acción concreta, no con una promesa.'},
      {q:'Un contacto acepta recibir la visita, pero vive solo y nunca cocina. ¿Qué haría un experto?',best:'Profundizar antes de agendar y explorar si existe un hogar cercano mejor calificado al que pueda recomendar.',good:'Agendar una visita corta para conocerlo mejor.',ok:'Mantenerlo en una lista secundaria y buscar más contexto.',risky:'Priorizarlo porque ya dijo que sí.',why:'Protege tiempo y recursos sin desechar la relación.',psych:'La aceptación no garantiza ajuste ni capacidad de decisión.',tip:'Calificar no es juzgar: es decidir dónde invertir una demostración completa.'},
      {q:'El prospecto pregunta: “¿Tengo que comprar algo?”. ¿Cuál respuesta es más completa?',best:'Aclarar que no existe obligación, mantener la curiosidad y regresar inmediatamente a escoger una fecha concreta.',good:'Decir que es una experiencia gratuita y que puede decidir después.',ok:'Explicar brevemente que conocerá una alternativa para su hogar.',risky:'Prometer que no se hablará de productos.',why:'Responde la preocupación real sin convertir la llamada en una presentación.',psych:'Está protegiendo su libertad y anticipando presión.',tip:'Reduce tensión y vuelve a la agenda.'},
      {q:'Después de una venta, el cliente cree que ya ganó el premio. ¿Qué conviene hacer?',best:'Volver a la fórmula completa, hacer que el cliente calcule qué visitas faltan y acordar la siguiente activación.',good:'Recordarle que necesita cuatro visitas y una venta.',ok:'Mostrar nuevamente el catálogo de premios para motivarlo.',risky:'Felicitarlo y dejar el seguimiento para después.',why:'La comprensión activa evita falsas expectativas y mantiene el programa vivo.',psych:'Ya siente que terminó y su motivación puede caer.',tip:'Haz que el cliente verbalice lo que falta.'}
    ];
    const speechStarts=['Señora Andrea, una pregunta:','Para asegurar que sea yo quien los visite,','Vamos a llamar primero a','Usted la saluda y le dice','Hola, mucho gusto. Estoy aquí con','La idea es llevarle el detalle','¿Qué día de esta semana','Perfecto, confirmemos entonces','Antes de terminar, regáleme','Si hoy no responde,'];
    const speechEnds=['¿prefiere que vaya alguien que ya conoce o una persona desconocida?','necesitamos dejarlos agendados de una vez.','la persona con quien usted tenga mayor confianza.','que quiere pasarme para contarle algo chévere.','la persona que decidió obsequiarle esta experiencia.','sin obligación y con todos los ingredientes incluidos.','podría recibirnos con su núcleo familiar?','fecha, hora y dirección completa.','un punto de referencia para llegar sin retrasos.','continuamos con el siguiente contacto sin perder el ritmo.'];
    const speechBank=Array.from({length:50},(_,i)=>{const s=speechStarts[i%10],e=speechEnds[i%10],wrong1=speechEnds[(i+3)%10],wrong2=speechEnds[(i+6)%10],wrong3=speechEnds[(i+8)%10];return {q:`Completa la intervención ${i+1}: “${s} …”`,best:e,good:wrong1,ok:wrong2,risky:wrong3,why:'La mejor opción conserva seguridad, brevedad y una acción concreta.',psych:'Las palabras deben reducir presión y aumentar compromiso.',tip:'No memorices por sonido; identifica la intención de la frase.'}});
    const marketProfiles=[['Casada, dos hijos, cocina a diario y trabaja','Prioridad alta con núcleo decisor presente'],['Vive solo, come fuera y no cocina','Profundizar y buscar una recomendación mejor calificada'],['Pareja joven, ambos trabajan y comparten decisiones','Agendar cuando ambos estén presentes'],['Docente, red familiar amplia y disfruta cocinar','Explorar primer círculo y colegas calificados'],['Pensionado, vive con su hija y cocina con frecuencia','Validar quién decide y quién estará presente']];
    function itemFrom(base,seed=counter++){return buildQuestion(base,level,seed)}
    function shell(title,how){stage.innerHTML=`<div class="game-head"><div><span class="game-kicker">Nivel ${levelLabel[level]}</span><h3>${title}</h3></div><span class="badge">Caso ${counter+1}</span></div>${how}<div class="question-zone"></div>`}
    function objections(){const b=bases[counter%bases.length];shell('Laboratorio de objeciones',renderHow('Escucha la objeción y elige la respuesta de mayor calidad.','Las cuatro opciones pueden ayudar; gana la que mejor valida, orienta y deja una acción concreta.',['Empatía','Intención','Siguiente paso'],level));renderMCQ(stage,itemFrom(b),g=>mastery.add('objections',g,level),objections)}
    function markets(){const [profile,best]=marketProfiles[counter%marketProfiles.length];const b={q:`Perfil: ${profile}. ¿Cuál es la decisión más profesional?`,best,good:'Agendar y completar la calificación durante la visita.',ok:'Solicitar más datos antes de definir prioridad.',risky:'Descartarlo o priorizarlo solo por una señal.',why:'La mejor decisión combina perfil, contexto y uso eficiente de recursos.',psych:'Calificar protege al asesor y también evita hacer perder tiempo al cliente.',tip:'Busca núcleo, uso, decisión y confianza.'};shell('Juego de mercados',renderHow('Analiza el perfil antes de invertir una demostración.','No existe “cliente perfecto”; debes reconocer el siguiente paso con mejor relación entre potencial y esfuerzo.',['Calificación','Prioridad','Mercado'],level));renderMCQ(stage,itemFrom(b),g=>mastery.add('markets',g,level),markets,'Otro perfil')}
    function errors(){const b={q:'El asesor acepta diez nombres sin preguntar relación, ocupación, convivencia ni hábitos. ¿Dónde está el error principal?',best:'Confundió llenar la hoja con construir un programa: perdió calidad antes de la instantánea.',good:'Debió hacer al menos dos preguntas de calificación antes de continuar.',ok:'Debió ordenar los nombres por cercanía y luego revisar información.',risky:'El único error fue no pedir más teléfonos.',why:'El error invisible ocurre antes de agendar: la calidad del programa se decide al pedir y calificar.',psych:'La prisa por “terminar” suele esconder incomodidad para profundizar.',tip:'La velocidad sin criterio crea cancelaciones más rápido.'};shell('Detecta el error',renderHow('Encuentra el error que más daño causa.','Algunas fallas son visibles; otras parecen eficientes, pero debilitan el paso siguiente.',['Causa raíz','Orden','Consecuencia'],level));renderMCQ(stage,itemFrom(b),g=>mastery.add('instant',g,level),errors)}
    function order(){
      const steps=['Puente','Premios','Pedir nombres','Calificar','Explicar programa','Instantánea y activación','Seguimiento','Mover programas'];let seq=shuffle(steps),picked=[];
      shell('Orden perfecto',renderHow('Reconstruye el sistema y resuelve el caso final.','Primero ordena. Si fallas, el punto exacto se marca en rojo. En niveles altos aparece una segunda decisión.',['Secuencia','Causa y efecto','Recuperación'],level));
      stage.querySelector('.question-zone').innerHTML=`<div class="order-game-v3">${seq.map(x=>`<button>${x}</button>`).join('')}</div><div class="chosen-order"></div><div class="game-feedback" hidden></div><button class="small-action reset-v3">Reiniciar</button>`;
      const buttons=[...stage.querySelectorAll('.order-game-v3 button')],chosen=stage.querySelector('.chosen-order'),fb=stage.querySelector('.game-feedback');
      buttons.forEach(b=>b.onclick=()=>{b.disabled=true;picked.push(b.textContent);chosen.innerHTML=picked.map((x,i)=>`<span>${i+1}. ${x}</span>`).join('');if(picked.length===8){const wrong=picked.findIndex((x,i)=>x!==steps[i]);if(wrong<0){fb.hidden=false;fb.className='game-feedback good';fb.innerHTML='✓ <strong>Orden perfecto.</strong> Ahora responde el caso de transferencia.';mastery.add('order',true,level);if(level!=='beginner'){setTimeout(()=>{const b={q:'Una visita se cancela después de la activación. ¿Qué mantiene la magia del orden?',best:'Hacer seguimiento, diagnosticar la causa y mover el programa con reemplazo o nueva fecha.',good:'Volver a llamar al mismo prospecto con otro horario.',ok:'Pedir al cliente un contacto adicional.',risky:'Reiniciar todo el programa desde el puente.',why:'No se retrocede por reflejo: se usa el paso de seguimiento para recuperar y mover.',tip:'El orden también enseña dónde corregir.'};stage.querySelector('.question-zone').insertAdjacentHTML('beforeend','<div class="order-followup"></div>');const wrap=stage.querySelector('.order-followup');wrap.className='question-zone order-followup';renderMCQ(wrap,itemFrom(b),g=>mastery.add('order',g,level),order,'Nuevo rompecabezas')},400)}}else{fb.hidden=false;fb.className='game-feedback bad';fb.innerHTML=`⚠ Fallaste en la posición <strong>${wrong+1}</strong>. Elegiste <em>${picked[wrong]}</em>, pero correspondía <strong>${steps[wrong]}</strong>.`;[...chosen.children].forEach((x,i)=>{if(i===wrong)x.classList.add('order-wrong')});mastery.add('order',false,level)}}});stage.querySelector('.reset-v3').onclick=order;
    }
    function speech(){const b=speechBank[counter%speechBank.length];shell('Completa el speech',renderHow('Completa, corrige o mejora una intervención.','El banco contiene 50 ejercicios. Principiante trabaja estructura; intermedio intención; experto precisión y psicología.',['Brevedad','Autoridad','Compromiso'],level));renderMCQ(stage,itemFrom(b),g=>mastery.add('instant',g,level),speech,'Siguiente speech')}
    function timer(){shell('Entrena tu instantánea',renderHow('Agenda en 1 minuto y 30 segundos.','Sigue las fases y termina con fecha, hora, dirección y núcleo familiar confirmado.',['Confianza','Compromiso','Agenda'],level));stage.querySelector('.question-zone').innerHTML=`<div class="timer-display" id="v3Timer">01:30</div><div class="timer-phase"><span>PREPARACIÓN</span><strong id="v3Cue">Explicación rápida → compromiso → fecha → dirección.</strong></div><div class="timer-progress"><i id="v3TimerBar"></i></div><button class="primary-action" id="v3StartTimer">Comenzar</button><div class="instant-checklist" hidden id="v3Checklist">${['Mencioné quién recomendó','Expliqué sin vender','Comprometí al cliente','Agendé fecha y hora','Confirmé dirección','Confirmé núcleo familiar'].map(x=>`<label><input type="checkbox"> ${x}</label>`).join('')}<button class="primary-action" id="v3ScoreTimer">Evaluar</button><div class="game-feedback" hidden></div></div>`;let left=90,t;const cues=[[90,'Genera confianza y menciona la recomendación.'],[70,'Explicación rápida: no vendas por teléfono.'],[50,'Compromete: pregunta qué día puede recibirte.'],[30,'Fecha, hora, dirección y quién estará presente.'],[10,'Confirma todo y termina la llamada.']];const display=stage.querySelector('#v3Timer'),cue=stage.querySelector('#v3Cue'),bar=stage.querySelector('#v3TimerBar');function paint(){display.textContent=`0${Math.floor(left/60)}:${String(left%60).padStart(2,'0')}`;bar.style.width=((90-left)/90*100)+'%';const c=cues.find(x=>x[0]===left);if(c)cue.textContent=c[1]}stage.querySelector('#v3StartTimer').onclick=()=>{clearInterval(t);left=90;paint();stage.querySelector('#v3Checklist').hidden=true;t=setInterval(()=>{left--;paint();if(left<=0){clearInterval(t);stage.querySelector('#v3Checklist').hidden=false;cue.textContent='Evalúa si saliste con una cita concreta.'}},1000)};stage.querySelector('#v3ScoreTimer').onclick=()=>{const c=[...stage.querySelectorAll('#v3Checklist input')],good=c.filter(x=>x.checked).length>=5,fb=stage.querySelector('#v3Checklist .game-feedback');fb.hidden=false;fb.className='game-feedback '+(good?'good':'bad');fb.textContent=good?'✓ Instantánea sólida: hubo agenda y compromiso.':'⚠ Repite: una llamada abierta no es una instantánea terminada.';mastery.add('instant',good,level)}}
    function mind(){const b=bases[(counter+2)%bases.length];shell('Piensa como un experto',renderHow('Interpreta antes de responder.','Elige la lectura que permite manejar la emoción y proteger el siguiente paso.',['Psicología','Intención','Criterio'],level));renderMCQ(stage,itemFrom(b),g=>mastery.add('objections',g,level),mind)}
    function quick(){mind()}
    function mentor(){errors()}
    const renderers={objections,markets,errors,order,speech,timer,mind,quick,mentor};
    document.getElementById('g414Difficulty').onclick=e=>{const b=e.target.closest('button');if(!b)return;level=b.dataset.level;document.querySelectorAll('#g414Difficulty button').forEach(x=>x.classList.toggle('active',x===b));counter=0;renderers[current]()};
    gym.querySelectorAll('[data-g414]').forEach(b=>b.onclick=()=>{gym.querySelectorAll('[data-g414]').forEach(x=>x.classList.toggle('active',x===b));current=b.dataset.g414;counter=0;renderers[current]()});
    objections();
  }

  // ---- Exámenes de 3 preguntas antes de avanzar + modal oscuro ----
  const stepExamBases=[
    {q:'¿Qué función cumple este paso dentro del orden?',best:'Preparar emocional y operativamente el siguiente paso sin forzar el proceso.',good:'Aumentar la probabilidad de avanzar con claridad.',ok:'Evitar errores comunes y mantener la conversación organizada.',risky:'Cumplir una parte obligatoria del guion.',why:'El valor de cada paso está en cómo prepara el siguiente.'},
    {q:'¿Qué señal demuestra que el paso quedó bien ejecutado?',best:'Existe una respuesta concreta del cliente que permite avanzar con confianza y criterio.',good:'El cliente comprende la idea principal y participa.',ok:'El asesor cubrió todos los puntos previstos.',risky:'La conversación terminó rápido y sin objeciones.',why:'La ejecución se mide por la calidad del avance, no por la velocidad.'},
    {q:'¿Cuál es el riesgo más importante al omitirlo?',best:'Debilitar la intención del siguiente paso y obligar al asesor a compensar con presión.',good:'Generar confusión o menor compromiso.',ok:'Perder información útil para continuar.',risky:'Alargar un poco la demostración.',why:'Saltarse un paso altera la psicología y la secuencia completa.'}
  ];
  document.addEventListener('click',e=>{const btn=e.target.closest('.learn-btn');if(!btn)return;e.preventDefault();e.stopImmediatePropagation();let pos=0,score=0;const modal=document.createElement('div');modal.className='learning-modal show v3-exam-modal';modal.innerHTML=`<div class="learning-dialog"><span class="game-kicker">EXAMEN ANTES DE AVANZAR</span><h3>Paso ${btn.dataset.learn}</h3><p>Responde tres preguntas. Todas las opciones son defendibles; elige la más completa.</p><div class="exam-v3-zone"></div></div>`;document.body.appendChild(modal);function ask(){if(pos>=3){modal.querySelector('.exam-v3-zone').innerHTML=`<div class="game-feedback ${score===3?'good':'bad'}">${score===3?'✓ Aprobado. Puedes marcar este paso como aprendido.':`Obtuviste ${score}/3. Debes responder las tres correctamente para avanzar.`}</div><button class="primary-action">${score===3?'Marcar aprendido':'Repetir examen'}</button>`;modal.querySelector('button').onclick=()=>{if(score===3){btn.classList.add('learned');btn.textContent='✓ Paso aprendido';modal.remove()}else{pos=0;score=0;ask()}};return}const item=buildQuestion(stepExamBases[pos],'expert',pos);const zone=modal.querySelector('.exam-v3-zone');zone.innerHTML='<div class="question-zone"></div>';renderMCQ(zone,item,g=>{if(g)score++;pos++},ask,'Siguiente pregunta')};ask();},true);

  // ---- Gimnasio de Ventas ----
  const sales=document.getElementById('gimnasioventas');
  if(sales){
    let level='beginner',game='objections',counter=0;const stage=document.getElementById('salesGymStage');const mastery=new SessionMastery('sales',['diagnosis','objections','closings','psychology']);
    const objections=['Está muy caro','Lo voy a pensar','Necesito hablarlo con mi pareja','Ya tengo utensilios','No tengo tiempo','No me gusta financiar','Quiero comparar','Ahora no es prioridad','No confío en las garantías','Prefiero comprar por internet'];
    const closingNames=['Alternativo','Resumen de valor','Balance decisional','Prueba de cierre','Elección guiada','Consecuencia','Compromiso progresivo','Presuntivo','Aislamiento de objeción','Silencio estratégico','Pregunta de escala','Visualización','Urgencia ética','Costo de no actuar'];
    function genSalesBase(type,i){const obj=objections[i%objections.length],close=closingNames[i%closingNames.length];if(type==='objections')return {q:`Cliente: “${obj}”. ¿Cuál es la mejor primera intervención?`,context:`Caso ${i+1} · La demostración fue positiva, pero el cliente todavía no ha verbalizado su preocupación completa.`,best:'Validar su comentario, hacer una pregunta diagnóstica breve y responder solo a la causa real antes de pedir una decisión.',good:'Reconocer la objeción y resumir los beneficios más importantes.',ok:'Ofrecer una alternativa de pago o configuración que reduzca la fricción.',risky:'Responder inmediatamente con argumentos para demostrar que está equivocado.',why:'Una objeción se rebate mejor después de diagnosticarla; responder demasiado pronto puede atacar la objeción equivocada.',psych:'Puede estar protegiendo dinero, autonomía, imagen o seguridad.',tip:'Pregunta: “Además de eso, ¿hay algo más que le impida avanzar hoy?”'};if(type==='closing')return {q:`¿Cómo aplicarías un cierre ${close} sin presionar?`,context:`El cliente reconoce valor, hizo preguntas de uso y financiación, pero aún no toma una decisión.`,best:'Conectar lo que el cliente dijo que valora con una pregunta concreta que le permita escoger el siguiente paso.',good:'Resumir los beneficios y preguntar si desea avanzar.',ok:'Presentar dos alternativas válidas para facilitar la decisión.',risky:'Asumir que comprará y llenar la orden sin confirmar.',why:'El cierre ético reduce incertidumbre y facilita una decisión consciente.',psych:'El cliente necesita seguridad para pasar del interés al compromiso.',tip:'Cerrar no es empujar; es ayudar a decidir.'};if(type==='psychology')return {q:'¿Qué señal emocional es más importante leer en este momento?',context:`El cliente dejó de preguntar, mira a su pareja y repite: “sí me gusta, pero…”.`,best:'La búsqueda de validación y el temor a decidir solo; conviene incluir a la pareja y explorar qué falta para sentirse seguros.',good:'La duda económica; conviene revisar las opciones de pago.',ok:'La necesidad de más información; conviene resumir el valor.',risky:'La falta de interés; conviene terminar la demostración.',why:'La mirada y el “pero” indican conflicto interno, no necesariamente rechazo.',psych:'Quiere conservar armonía y evitar arrepentimiento.',tip:'Pregunta a ambos: “¿Qué necesitarían tener claro para sentirse tranquilos con la decisión?”'};if(type==='diagnosis')return {q:'¿Cuál es el diagnóstico más completo de esta demostración?',context:'El asesor habló 70% del tiempo, mostró toda la tecnología, respondió cada pregunta, pero nunca preguntó prioridades ni impacto en el hogar.',best:'Hubo buena información, pero faltó descubrimiento; el cliente conoció el producto sin conectar el valor con su necesidad personal.',good:'La presentación fue demasiado larga y necesita mayor participación.',ok:'Faltaron preguntas de prueba de cierre durante la demostración.',risky:'El problema principal fue no ofrecer un descuento.',why:'Sin diagnóstico, incluso una demostración técnica excelente puede sentirse genérica.',psych:'El cliente aprende, pero no necesariamente se ve a sí mismo usando la solución.',tip:'Una demo fuerte alterna pregunta, experiencia, confirmación y avance.'};return {q:'El cliente virtual se muestra desconfiado y pregunta por tercera vez por la garantía. ¿Qué haces?',context:'Estado del cliente: 🤨 Desconfiado · Interés 62% · Paciencia 48%',best:'Detener el avance, reconocer que la seguridad es importante, comprobar qué parte de la garantía le preocupa y responder con evidencia clara.',good:'Explicar nuevamente la garantía con más detalle.',ok:'Mostrar testimonios y documentación de respaldo.',risky:'Cambiar al cierre para evitar que siga dudando.',why:'La repetición indica que la emoción no fue resuelta, aunque la información ya se haya dado.',psych:'Teme equivocarse o no poder reclamar después.',tip:'No repitas más fuerte; pregunta mejor.'};}
    const banks={};['objections','closing','psychology','demo','diagnosis'].forEach(t=>banks[t]=Array.from({length:50},(_,i)=>genSalesBase(t,i)));
    function shell(title,how){stage.innerHTML=`<div class="game-head"><div><span class="game-kicker">Nivel ${levelLabel[level]}</span><h3>${title}</h3></div><span class="badge">${counter+1} / 50</span></div>${how}<div class="question-zone"></div>`}
    function play(type){const names={objections:'Rebata la objeción',closing:'El mejor cierre',psychology:'Psicología de compra',demo:'Demo virtual',diagnosis:'Diagnóstico comercial'};const evals={objections:['Escucha','Diagnóstico','Respuesta'],closing:['Momento','Ética','Decisión'],psychology:['Emoción','Señales','Adaptación'],demo:['Conducción','Estado del cliente','Tip en vivo'],diagnosis:['Causa raíz','Impacto','Corrección']};shell(names[type],renderHow(names[type],type==='demo'?'Cada respuesta modifica el estado emocional del cliente. Usa los tips para recuperar confianza, paciencia e interés.':'Resuelve el caso y compara opciones plausibles. La mejor respuesta integra psicología, estrategia y siguiente paso.',evals[type],level));const base=banks[type][counter%50],item=buildQuestion(base,level,counter);if(type==='demo'){stage.querySelector('.question-zone').insertAdjacentHTML('beforebegin',`<div class="client-state"><span class="state suspicious">🤨 Desconfiado</span><span>Interés <b>${55+(counter%25)}%</b></span><span>Paciencia <b>${40+(counter%30)}%</b></span><p>💡 Tip: reduce la velocidad, valida la emoción y formula una pregunta antes de continuar.</p></div>`)}renderMCQ(stage,item,g=>{const area=type==='closing'?'closings':type==='demo'?'psychology':type;mastery.add(area,g,level);if(type==='demo'){const state=stage.querySelector('.client-state .state');state.className='state '+(g?'receptive':'impatient');state.textContent=g?'🙂 Más receptivo':'😣 Impaciente';}},()=>{counter=(counter+1)%50;play(type)},'Siguiente ejemplo')}
    document.getElementById('salesDifficulty').onclick=e=>{const b=e.target.closest('button');if(!b)return;level=b.dataset.level;sales.querySelectorAll('#salesDifficulty button').forEach(x=>x.classList.toggle('active',x===b));counter=0;play(game)};
    sales.querySelectorAll('[data-sales-game]').forEach(b=>b.onclick=()=>{sales.querySelectorAll('[data-sales-game]').forEach(x=>x.classList.toggle('active',x===b));game=b.dataset.salesGame;counter=0;play(game)});play('objections');
  }
})();
